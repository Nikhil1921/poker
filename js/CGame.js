function CGame(oData){
    ...
    this._calculateTotalWin = function(){
        if(_iHandDealer === NO_HAND && _szHandResult !== "dealer"){
            ...
            var iRaiseBet = (_oSeat.getBetAnte() * 2);
            _iTotWin = iRaiseBet + (iRaiseBet * PAYOUT_MULT[_iHandPlayer]) + iRaiseBet;
        }else if(_szHandResult === "dealer" && _iHandDealer !== NO_HAND){
            _iTotWin = 0;
        }else{
            //STAND OFF
       ...
       _oCardContainer.removeAllChildren();
        ...
        if(iRandWin > WIN_OCCURRENCE){
            ...
            do{
                ...
                var oRetDealer = _oHandEvaluator.evaluate(_aDealerCardsInfo,true);
                var oRetPlayer = _oHandEvaluator.evaluate(_aPlayerCardsInfo,false);
    ...
    this.onFold = function(){
        ...
        // when user fold the game;
        // $(s_oMain).trigger("save_score", [_oSeat.getCredit()]);
        fold(_szHandResult, _oSeat);
    };
    ...
    this.onRaise = function(){
        ...
        _iCurDealerCardShown = 0;
        this._showNextDealerCard();
        // when user raise the game money;
        raise(_szHandResult, _oSeat);
        // $(s_oMain).trigger("save_score", [_oSeat.getCredit()]);
    };
